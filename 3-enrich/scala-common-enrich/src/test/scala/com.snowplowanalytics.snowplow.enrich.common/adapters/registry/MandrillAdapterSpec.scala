...
class MandrillAdapterSpec extends Specification with DataTables with ValidatedMatchers {
  ...
  def e2 =
    "SPEC NAME" || "STRING TO PROCESS" | "EXPECTED OUTPUT" |
      "Failure, empty events string" !! "mandrill_events=" ! InputDataAdapterFailure(
        "body",
        "mandrill_events=".some,
        "`mandrill_events` field is empty"
      ) |
      "Failure, too many key-value pairs" !! "mandrill_events=some&mandrill_extra=some" ! InputDataAdapterFailure(
        "body",
        "mandrill_events=some&mandrill_extra=some".some,
        "body should have size 1: actual size 2"
      ) |
      "Failure, incorrect key" !! "events_mandrill=something" ! InputDataAdapterFailure(
        "body",
  ...
  def e8 = {
    ...
    MandrillAdapter.toRawEvents(payload, SpecHelpers.client).value must beInvalid(
      NonEmptyList.one(
        InputDataAdapterFailure("contentType", ct, "expected application/x-www-form-urlencoded")
      )
    )
  }
}
